...
import { Plus, Search, Pencil, Trash2, AlertCircle, Loader2, Upload, Image as ImageIcon, X } from 'lucide-react';
...
export default function Products() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Produk' : 'Produk Baru'} size="lg">
        <form onSubmit={onSubmit} className="space-y-3 text-sm">
          {formError && (
            <div className="bg-rose-50 border border-rose-200 text-rose-700 text-xs rounded-md px-3 py-2">{formError}</div>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Field label="Harga Modal">
              <input type="number" min="0" value={form.costPrice} onChange={(e) => setForm({ ...form, costPrice: e.target.value })} className={inputCls} />
            </Field>
          ...
          <div>
            <span className="block text-xs font-semibold text-slate-700 mb-1">Gambar Produk</span>
          ...
          <div className="flex gap-4">
            ...
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={form.showOnline} onChange={(e) => setForm({ ...form, showOnline: e.target.checked })} />
              <span>Tampil di Toko Online</span>
            </label>
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => setModalOpen(false)} className="px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-lg">
              Batal
            </button>
